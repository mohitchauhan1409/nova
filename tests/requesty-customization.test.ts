...
import { requestyPolicyDraftSnapshot, requestyPromptSnapshot } from './fixtures/requesty';

describe('Requesty customer customization', () => {
  it('registers an isolated, verified profile with substantial safe flows', () => {
    ...
    expect(requestyProfile.instructions).toContain('concrete confirmation');
    expect(requestyProfile.instructions).toContain('Never invent request IDs');
    expect(requestyProfile.instructions).toContain('Never add credits');
  });
  ...
  it('provides synthetic fixtures without live keys or real spend', () => {
    ...
    expect(requestyPromptSnapshot.text).toContain('Model optional');
    expect(JSON.stringify([requestyPolicyDraftSnapshot,requestyPromptSnapshot])).not.toMatch(/sk-[A-Za-z0-9]{8,}/);
  });
});
