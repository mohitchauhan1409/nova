// Isolated presentation/state test; never connects to the user's browser or Bolna.
import {chromium} from 'playwright';
import {mkdir,writeFile} from 'node:fs/promises';
import assert from 'node:assert/strict';
const browser=await chromium.launch({headless:true});
const session={id:'visual',url:'https://platform.bolna.ai/workflows',title:'Workflows · Bolna',status:'ready',messages:[],traces:[],experience:{name:'Bolna',accent:'#3862e9',greeting:'',suggestions:[{title:'Build a workflow',prompt:'Help me build a workflow'},{title:'Prepare a campaign',prompt:'Help me prepare a campaign'}]}};
try{
 ...
 await mkdir('artifacts/bolna/experience-refresh',{recursive:true});
 await page.screenshot({path:'artifacts/bolna/experience-refresh/panel-welcome.png'});
 ...
 assert.equal(await page.locator('[data-status=running]').count(),1);await page.screenshot({path:'artifacts/bolna/experience-refresh/panel-working.png'});
 ...
 await page.evaluate(s=>(window as any).novaTestSend({...s,url:'https://workspace.example/workflows',experience:{...s.experience,name:'Workspace',accent:'#7360db'}}),session);
 await page.waitForFunction(()=>document.querySelector('.np-app')?.getAttribute('data-site')==='workspace.example');
 assert.equal(await page.locator('.np-app').evaluate(e=>getComputedStyle(e).backgroundColor),'rgb(250, 249, 247)','Other sites retain the core appearance');
 await writeFile('artifacts/bolna/experience-refresh/ui-checks.json',JSON.stringify({ok:true,checks:['Bolna theme scoped to exact hostname','other sites retain core theme','chat default','website identity','live step updates','no false verification for failure','320–480px no horizontal overflow','all active steps visible','new steps follow automatically','reading older steps preserved','completed answer remains visible']},null,2));console.log('Panel identity, action states and responsive layout passed.');
