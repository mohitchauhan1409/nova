...
const session=sessions.find(s=>s.siteId==='creem');
if(!session)throw new Error('No Creem Nova session exists.');
const report={id:session.id,status:session.status,url:session.url,model:session.model,usage:session.usage,messages:session.messages,traces:session.traces,actionSteps:session.actionSteps,startedAt:session.startedAt};
