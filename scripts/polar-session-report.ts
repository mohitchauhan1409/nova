...
const session=sessions.filter(s=>s.siteId==='polar').sort((a,b)=>b.startedAt-a.startedAt)[0];
if(!session)throw new Error('No Polar Nova session exists.');
...
await mkdir('artifacts/polar/rehearsals',{recursive:true});
await writeFile(`artifacts/polar/rehearsals/${label}.json`,JSON.stringify(report,null,2),{mode:0o600});
console.log(JSON.stringify({status:report.status,messages:report.messages.slice(-2).map(m=>({role:m.role,at:m.at,text:m.text.slice(0,500),questionCard:m.clarification?.title})),steps:report.actionSteps?.slice(-5).map(s=>({at:s.at,title:s.title,status:s.status,detail:s.detail})),recent:report.traces.slice(-2),usage:report.usage},null,2));
