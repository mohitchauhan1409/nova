...
export function PanelApp() {
  ...
  const suggestions = [
    {icon:Sparkles,title:'Help me get started',detail:'A question, a plan, then the next step',prompt:'What can you help me do on this page?',send:true},
    ...(experience?.suggestions.length?experience.suggestions:[{title:'Find something here',prompt:'Find '},{title:'Help with a task',prompt:'Help me '}]).map((item,i)=>({...item,icon:i===0?Search:ListChecks,detail:'Tell me the goal. I’ll handle the steps.',send:false})),
  ];
  ...
  return <main className="np-app" data-site={hostname} aria-label="Nova side panel" style={{'--site-accent':palette.accent,'--site-ink':palette.ink,'--site-soft':palette.soft} as CSSProperties}>
    <header className="np-header"><div className="np-brand"><span className="np-brand-mark"><NovaMark size={25}/></span><div><strong>Nova</strong><small>YOUR AI ASSISTANT</small></div></div><div className="np-header-actions"><button className="np-icon-button" aria-label="New conversation" title="New conversation" onClick={newChat} disabled={!usable}><SquarePen size={17}/></button><button className="np-icon-button" aria-label="Close Nova panel" title="Close panel" onClick={panel.close}><PanelRightClose size={18}/></button></div></header>
    <div className="np-tabs" role="tablist" aria-label="Conversation mode"><button role="tab" id="chat-tab" aria-controls="chat-panel" aria-selected={view === 'chat'} onClick={chooseChat}><MessageCircle size={14}/> Chat</button><button role="tab" id="voice-tab" aria-controls="voice-panel" aria-selected={view === 'voice'} onClick={chooseVoice} disabled={!usable}><AudioLines size={15}/> Live talk {live && <i/>}</button><span>ASK. PLAN. DONE.</span></div>
    ...
    : <>
      {view === 'chat' ? <section id="chat-panel" role="tabpanel" aria-labelledby="chat-tab" className="np-chat-panel">
        ...
        <div className="np-composer-area">
          ...
          <form className={`np-composer ${busy ? 'is-working' : ''}`} onSubmit={e => { e.preventDefault(); submit(); }}><textarea ref={input} aria-label="Message Nova" placeholder={session?.awaitingAnswer ? 'Tell Nova a little more…' : 'Ask anything, or ask me to do it…'} rows={2} maxLength={8000} value={draft} disabled={!usable} onChange={e => setDraft(e.target.value)} onKeyDown={e => { if (e.key === 'Enter' && !e.shiftKey && !e.nativeEvent.isComposing) { e.preventDefault(); submit(); } }}/><div className="np-composer-tools"><span className="np-page-context"><Globe2 size={12}/><span title={siteName}>{siteName}</span></span><div><button type="button" className="np-icon-button" aria-label="Start live talk" title="Talk to Nova" onClick={chooseVoice} disabled={!usable}><AudioLines size={19}/></button><button type="submit" className="np-send" aria-label="Send message" disabled={!draft.trim() || !usable}><ArrowUp size={17}/></button></div></div></form>
          <div className="np-composer-foot"><span>{busy ? 'You can interrupt or change direction.' : 'Your goal. My next step.'}</span><button aria-label="Toggle activity" aria-expanded={activity} onClick={() => setActivity(!activity)}><ListChecks size={12}/><span>Activity</span></button></div>
        </div>
      </section>
