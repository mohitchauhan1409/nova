...
Use [activation.md](activation.md) for exact isolated-checkout commands reproducing the recorded extension/frontend and backend pair, including both recording flags. The steps below describe a general new build, not the frozen replay recipe.
...
5. Confirm the saved baseline, dark theme, native side panel, launcher hide/show, focus, actual editor input and one real cursor click/scroll through a short capture before rehearsal. The final raw capture and local/independent export/cursor QA subsequently passed; see the final delivery evidence below.
...
| Capability | Evidence at pre-capture checkpoint | Historical status before the final take |
| --- | --- | --- |
...
Independent business checks are complete and recorded in [verification/final-take-receipts.json](verification/final-take-receipts.json) and the allowlisted [synthetic object manifest](verification/synthetic-objects.json). Nova itself could not verify saved tags in the grid; the coordinator supplied that separate proof. Media edits, sampled actual Nova-cursor checks, approved Bolna tap/cue verification and audiovisual QA passed; see [local media QA](evidence/final-qa-report.md) and [independent acceptance](evidence/independent-final-acceptance.md). Independent private Git/LFS recovery passed at `98ed06a`, with all three complete recovered files matching local originals and remote pointers; see [remote backup verification](evidence/remote-backup-verification.json). No verification report is inferred from a successful push alone. Keep loaded runtime identity distinct from later documentation commits. Earlier saved outcomes with recovery must not be labeled clean rehearsals, and the final's verified-step count must not hide its planning errors or user steering.

## Authorized scratch cleanup

After independent remote recovery passed, the coordinator authorized removal of exactly **323 generated scratch media files** (251,176,740 bytes) and **3 verified duplicate recovery blobs** (274,819,835 bytes). Cleanup removed **326 files / 525,996,575 bytes**. All three primary recordings were retained and their complete SHA-256 hashes rechecked after cleanup. The 561 existing noncandidate private records and 37 runtime/build/config files checked during cleanup were unchanged. Reports, JSON, scripts, logs, authorized credentials, pairing/site state, build outputs and shared Git/LFS data were preserved. Exact paths, identities and totals are in [cleanup-manifest.json](evidence/cleanup-manifest.json). No enclosing evidence directory or unrelated startup was removed.
