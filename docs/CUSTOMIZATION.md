...
Generated `web/dist`, `web/dist-extension`, release ZIPs, browser data, reports, screenshots, recordings, keys and `.env` files are ignored. A fresh clone builds its own extension and supplies its own provider credentials. A source repository is not a hosted deployment.
