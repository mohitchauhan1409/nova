...
[final-click-ledger.json](final-click-ledger.json) retains **37 unique full-session receipts: 25 Nova and12 operator**, including launcher/composer clicks before the first message. Deduplication uses epoch, actor, target and button across39 collector exports. Task-filtered summaries alone would omit the opening operator clicks. Each receipt maps to its nearest native source frame, inspected visually, then through the exact EDL into [final-click-cues.json](final-click-cues.json). No click is invented for Enter submissions, typing, thinking, scrolling or the final no-op review response.
