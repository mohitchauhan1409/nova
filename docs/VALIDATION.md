...
The client additions are isolated in site profiles, scoped theme hooks, client fixtures/scripts and `BOLNA-*` documents. Existing local profiles are preserved. Historical live workflow observations are retained in [the original validation log](BOLNA-VALIDATION-HISTORY.md) and [workflow lab](BOLNA-WORKFLOW-LAB.md); reports remain local and ignored. The two retained recordings are archived in Git LFS only on this branch; see [recordings and recovery](BOLNA-RECORDINGS.md).
