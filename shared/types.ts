...
export type Snapshot = { textRegions?: {ref:string;text:string;elementRefs:string[]}[]; id: string; url: string; title: string; text: string; elements: ElementRef[]; viewport: { width: number; height: number; scrollX?: number; scrollY?: number; zoom?: number }; theme: { color: string; font: string; scheme?: 'light' | 'dark' }; frames: number; blocked?: string; capturedAt: number; capabilities?: string[]; observation?: {totalControls:number;omittedControls:number;viewportFirst:boolean;sensitiveFieldsPresent?:boolean} };
