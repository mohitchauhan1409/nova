...
Open the intended inbox and inspect Drafts before starting, using the exact subject, recipient and inbox to avoid duplicates. Compose opens To (email), Subject and a message textarea. If required details are missing, collect recipient, purpose/facts and tone in one focused card; do not ask for optional fields that are unnecessary. Enter the reserved recipient, subject and agreed content. Close autosaves the draft in the inspected console; do not click Send or scheduling controls. After Close, open Drafts and the saved subject row. Edit Draft must show the actual persisted To, Subject and body. Verify all requested facts and that it is still an unscheduled draft. A disappeared composer or a toast alone is not proof of persistence. If absent, inspect current Drafts once and recover the unsaved task carefully; do not repeatedly create copies.
