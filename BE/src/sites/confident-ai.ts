...
export const confidentAiProfile: SiteProfile = {
  ...
  instructions: [
    ...
    'For an existing golden, the row pencil at the left beside its checkbox opens the Edit Golden form. Prefer this observed form for reliable Expected Output edits instead of double-clicking body-text cells; do not horizontally scroll right to find the pencil. Identify the exact row by its observed ID and full Input, use the currently visible pencil, and inspect the opened form before editing only the requested fields. Save once and reopen that same golden to verify persistence. An unlabeled pencil may require the shared control-review approval; do not bypass that review or assume approval from this guide.',
