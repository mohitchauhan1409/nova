...
export const confidentAiProfile: SiteProfile = {
  ...
  instructions: [
    ...
    'The coordinator can open Edit Golden with the left row pencil beside its checkbox, but Nova has not reliably opened that editor: an approved visual pencil click and an approved inline-cell double-click produced no verified editor. Existing-row editing is excluded from the current demonstration; do not describe it as validated or bypass a control review. New Golden creation has saved and reopened distinct cases successfully. For a requested new case, inspect current full inputs first and create it only when no matching case exists; preserve all existing records.',
